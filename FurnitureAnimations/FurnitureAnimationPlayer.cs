...
{
    public enum EaseMode { Linear, Global, PerFrame }
    ...
    public class FurnitureAnimationPlayer : MonoBehaviour
    {
        ...
        public void Play(CharacterCustomization character, string animationName, Furniture furniture, PoseData poseConfig)
        {
            ...
            {
                Plugin.Log.LogError($"[Player] Анимация на диске не найдена: {assetPath}");
                Destroy(this);
                return;
            }
            ...
            {
                Plugin.Log.LogError($"[Player] Ошибка парсинга JSON: {ex.Message}");
                Destroy(this);
                return;
            }

            if (_animData?.deltas == null || _animData.deltas.Count == 0)
            {
                Plugin.Log.LogError($"[Player] В файле '{animationName}' нет доступных дельт!");
                Destroy(this);
                return;
            }

            // Инициализируем сторонние менеджеры, прикрепляя их к этому же GameObject
            ...
            Dictionary<string, BoneDelta> firstFrameDatas = _animData.deltas[0].boneDatas;
        ...
        public void ChangeSpeed(float delta) => _speedModifier = Mathf.Clamp(_speedModifier + delta, 0.1f, 3.0f);
        public float GetSpeed() => _speedModifier;
        public void ToggleEaseMode() => _currentEaseMode = (EaseMode)(((int)_currentEaseMode + 1) % 3);
        ...
        private float ApplyEasing(float fraction)
        {
            ...
            {
                case EaseMode.Global: return fraction * fraction * (3f - 2f * fraction);
                case EaseMode.PerFrame: return Mathf.SmoothStep(0f, 1f, fraction);
        ...
        private Vector3 ArrayToVector3(float[] a) => (a == null || a.Length < 3) ? Vector3.zero : new Vector3(a[0], a[1], a[2]);
        ...
        private void AbsoluteSkeletalReset(Dictionary<string, BoneDelta> firstFrameBoneDatas)
        {
            ...
            {
                if (!_boneCache.TryGetValue(boneName, out Transform boneTrans)) continue;
                ...
                {
                    if (delta.endRot != null && delta.endRot.Length >= 4) boneTrans.localRotation = new Quaternion(delta.endRot[0], delta.endRot[1], delta.endRot[2], delta.endRot[3]);
                    else if (delta.endRot != null && delta.endRot.Length == 3) boneTrans.localRotation = Quaternion.Euler(delta.endRot[0], delta.endRot[1], delta.endRot[2]);
                    if (delta.endPos != null && delta.endPos.Length >= 3) boneTrans.localPosition = new Vector3(delta.endPos[0], delta.endPos[1], delta.endPos[2]);
                }
            }
        }
